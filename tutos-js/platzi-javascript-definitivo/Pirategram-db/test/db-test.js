...
test.beforeEach('setup database', async t => {
  const dbName = `pirategram_${uuid.v4()}`
...
test.afterEach.always('disconnect and cleanup database', async t => {
  let db = t.context.db
...
test('list images by user', async t => {
  ...
  let random = Math.round(Math.random() * images.lenght)
